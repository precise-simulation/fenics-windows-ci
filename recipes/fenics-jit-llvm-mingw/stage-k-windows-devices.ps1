...
# Run #184 requalified Stage J and measured every windows.devices* header as
# unobserved across all 27 Phase 5 compile dependency closures. Prune only this
# 2.865 MiB Windows Runtime subfamily; the other WinRT metadata stays intact.
$matches = @(
    Get-ChildItem -LiteralPath $include -File |
        Where-Object { $_.Name -like "windows.devices*" } |
        Sort-Object Name
)
if ($matches.Count -eq 0) {
    throw "Stage K found no windows.devices* headers; upstream layout may have changed"
}
...
if ($remaining.Count -ne 0) {
    throw "Stage K windows.devices* headers remain: $($remaining.Name -join ', ')"
}

$stageJReport = Join-Path $root "minimization-stage-j.json"
if (-not (Test-Path -LiteralPath $stageJReport -PathType Leaf)) {
    throw "Stage J minimization report missing before Stage K: $stageJReport"
}

$report = [ordered]@{
    stage = "stage-k"
    parent_stage = "stage-j"
    evidence_run = 184
    evidence_compile_trace_count = 27
    family = "windows_runtime_devices"
    patterns = @("include/windows.devices*")
    removed_file_count = $removed.Count
    removed_bytes = $removedBytes
    removed_mib = [math]::Round($removedBytes / 1MB, 3)
    removed = $removed
}
$reportPath = Join-Path $root "minimization-stage-k.json"
$report | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $reportPath -Encoding UTF8

$metadataPath = Join-Path $root "metadata.json"
if (-not (Test-Path -LiteralPath $metadataPath -PathType Leaf)) {
    throw "LLVM-MinGW metadata missing before Stage K: $metadataPath"
}
$metadata = Get-Content -LiteralPath $metadataPath -Raw | ConvertFrom-Json
if ([string]$metadata.minimization_stage -ne "stage-j") {
    throw "Stage K expected Stage J metadata, got: $($metadata.minimization_stage)"
}
$metadata.minimization_stage = "stage-k"
$metadata.minimization_report = "minimization-stage-k.json"
$metadata | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $metadataPath -Encoding UTF8

# Recreate Stage J's manifest/size files after this additional pruning so they
...
if ($env:PHASE6_VS2022_JIT_GATE_BYTES) {
    ...
    if ($payloadBytes -gt $phase6GateBytes) {
        throw "Phase 6 size gate failed after Stage K: staged payload $payloadBytes bytes exceeds 50% ceiling $phase6GateBytes bytes"
    }
...
Write-Host "LLVM-MinGW minimization stage-k"
Write-Host "  Stage K removed files: $($removed.Count)"
Write-Host "  Stage K removed MiB: $([math]::Round($removedBytes / 1MB, 2))"
Write-Host "  payload MiB after Stage K: $([math]::Round($payloadBytes / 1MB, 2))"
