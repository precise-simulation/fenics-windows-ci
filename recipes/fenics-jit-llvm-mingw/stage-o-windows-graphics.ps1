...
# Run #188 requalified Stage N and measured every windows.graphics* header as
# unobserved across all 27 Phase 5 compile dependency closures. Prune only
# this 0.802 MiB Windows Runtime subfamily; the other WinRT metadata stays
# intact.
$matches = @(
    Get-ChildItem -LiteralPath $include -File |
        Where-Object { $_.Name -like "windows.graphics*" } |
        Sort-Object Name
)
if ($matches.Count -eq 0) {
    throw "Stage O found no windows.graphics* headers; upstream layout may have changed"
}
...
if ($remaining.Count -ne 0) {
    throw "Stage O windows.graphics* headers remain: $($remaining.Name -join ', ')"
}

$stageNReport = Join-Path $root "minimization-stage-n.json"
if (-not (Test-Path -LiteralPath $stageNReport -PathType Leaf)) {
    throw "Stage N minimization report missing before Stage O: $stageNReport"
}

$report = [ordered]@{
    stage = "stage-o"
    parent_stage = "stage-n"
    evidence_run = 188
    evidence_compile_trace_count = 27
    family = "windows_runtime_graphics"
    patterns = @("include/windows.graphics*")
    removed_file_count = $removed.Count
    removed_bytes = $removedBytes
    removed_mib = [math]::Round($removedBytes / 1MB, 3)
    removed = $removed
}
$reportPath = Join-Path $root "minimization-stage-o.json"
$report | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $reportPath -Encoding UTF8

$metadataPath = Join-Path $root "metadata.json"
if (-not (Test-Path -LiteralPath $metadataPath -PathType Leaf)) {
    throw "LLVM-MinGW metadata missing before Stage O: $metadataPath"
}
$metadata = Get-Content -LiteralPath $metadataPath -Raw | ConvertFrom-Json
if ([string]$metadata.minimization_stage -ne "stage-n") {
    throw "Stage O expected Stage N metadata, got: $($metadata.minimization_stage)"
}
$metadata.minimization_stage = "stage-o"
$metadata.minimization_report = "minimization-stage-o.json"
...
if ($env:PHASE6_VS2022_JIT_GATE_BYTES) {
    ...
    if ($payloadBytes -gt $phase6GateBytes) {
        throw "Phase 6 size gate failed after Stage O: staged payload $payloadBytes bytes exceeds 50% ceiling $phase6GateBytes bytes"
    }
...
Write-Host "LLVM-MinGW minimization stage-o"
Write-Host "  Stage O removed files: $($removed.Count)"
Write-Host "  Stage O removed MiB: $([math]::Round($removedBytes / 1MB, 2))"
Write-Host "  payload MiB after Stage O: $([math]::Round($payloadBytes / 1MB, 2))"
