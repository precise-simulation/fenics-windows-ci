...
if not exist "%JIT_ROOT%\minimization-stage-ao.json" exit /b 1
if not exist "%JIT_ROOT%\minimization-stage-ap.json" exit /b 1
if not exist "%JIT_ROOT%\minimization-stage-aq.json" exit /b 1
if not exist "%JIT_ROOT%\minimization-stage-ar.json" exit /b 1
if not exist "%JIT_ROOT%\minimization-stage-as.json" exit /b 1
if not exist "%JIT_ROOT%\minimization-stage-at.json" exit /b 1
...
powershell.exe -NoProfile -Command "$meta = Get-Content -LiteralPath '%JIT_ROOT%\metadata.json' -Raw | ConvertFrom-Json; if ([string]$meta.minimization_stage -ne 'stage-av' -or [string]$meta.minimization_report -ne 'minimization-stage-av.json' -or [string]$meta.library_minimization_stage -ne 'stage-ab') { exit 1 }"
...
powershell.exe -NoProfile -Command "$names = @('qedit.h','qedit.idl'); $bad = @(Get-ChildItem -LiteralPath '%JIT_ROOT%\include' -File | Where-Object { $_.Name -in $names }); if ($bad.Count -ne 0) { $bad.FullName; exit 1 }"
if errorlevel 1 exit /b 1
powershell.exe -NoProfile -Command "$names = @('amstream.h','amstream.idl'); $bad = @(Get-ChildItem -LiteralPath '%JIT_ROOT%\include' -File | Where-Object { $_.Name -in $names }); if ($bad.Count -ne 0) { $bad.FullName; exit 1 }"
if errorlevel 1 exit /b 1
powershell.exe -NoProfile -Command "$names = @('evr.h','evr.idl','evr9.h','evr9.idl'); $bad = @(Get-ChildItem -LiteralPath '%JIT_ROOT%\include' -File | Where-Object { $_.Name -in $names }); if ($bad.Count -ne 0) { $bad.FullName; exit 1 }"
if errorlevel 1 exit /b 1
powershell.exe -NoProfile -Command "$names = @('taskschd.h','taskschd.idl'); $bad = @(Get-ChildItem -LiteralPath '%JIT_ROOT%\include' -File | Where-Object { $_.Name -in $names }); if ($bad.Count -ne 0) { $bad.FullName; exit 1 }"
if errorlevel 1 exit /b 1
powershell.exe -NoProfile -Command "$names = @('wuapi.h','wuapi.idl'); $bad = @(Get-ChildItem -LiteralPath '%JIT_ROOT%\include' -File | Where-Object { $_.Name -in $names }); if ($bad.Count -ne 0) { $bad.FullName; exit 1 }"
if errorlevel 1 exit /b 1
powershell.exe -NoProfile -Command "$names = @('dvdif.h','dvdif.idl'); $bad = @(Get-ChildItem -LiteralPath '%JIT_ROOT%\include' -File | Where-Object { $_.Name -in $names }); if ($bad.Count -ne 0) { $bad.FullName; exit 1 }"
if errorlevel 1 exit /b 1
powershell.exe -NoProfile -Command "$names = @('vmr9.h','vmr9.idl'); $bad = @(Get-ChildItem -LiteralPath '%JIT_ROOT%\include' -File | Where-Object { $_.Name -in $names }); if ($bad.Count -ne 0) { $bad.FullName; exit 1 }"
if errorlevel 1 exit /b 1
powershell.exe -NoProfile -Command "$names = @('amvideo.h','amvideo.idl'); $bad = @(Get-ChildItem -LiteralPath '%JIT_ROOT%\include' -File | Where-Object { $_.Name -in $names }); if ($bad.Count -ne 0) { $bad.FullName; exit 1 }"
if errorlevel 1 exit /b 1
