...
# Run #225 requalified Stage AS across all 27 Phase 5 compile dependency
# closures. The DirectShow DVD interface root header/IDL pair below was
# unobserved in every trace: 2 files / 0.118 MiB. Use exact names and bytes so
...
$matches = @(
    foreach ($name in $names) {
        ...
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
            throw "Stage AT expected run #225 candidate header missing: $name"
        }
        Get-Item -LiteralPath $path
    }
)
if ($matches.Count -ne 2) {
    throw "Stage AT expected 2 DirectShow DVD interface files from run #225 evidence, found $($matches.Count)"
}

$expectedBytes = [int64]124208
$actualBytes = [int64](($matches | Measure-Object Length -Sum).Sum)
if ($actualBytes -ne $expectedBytes) {
    throw "Stage AT expected $expectedBytes bytes from run #225 evidence, found $actualBytes; refusing changed candidate set"
}
...
foreach ($name in $requiredHeaders) {
    if (-not (Test-Path -LiteralPath (Join-Path $include $name) -PathType Leaf)) {
        throw "Stage AT safety header missing before pruning: $name"
    }
}
...
foreach ($name in $names) {
    if (Test-Path -LiteralPath (Join-Path $include $name)) {
        throw "Stage AT DirectShow DVD interface file remains after pruning: $name"
    }
}
foreach ($name in $requiredHeaders) {
    if (-not (Test-Path -LiteralPath (Join-Path $include $name) -PathType Leaf)) {
        throw "Stage AT safety header missing after pruning: $name"
    }
}

$stageASReport = Join-Path $root "minimization-stage-as.json"
if (-not (Test-Path -LiteralPath $stageASReport -PathType Leaf)) {
    throw "Stage AS minimization report missing before Stage AT: $stageASReport"
}

$report = [ordered]@{
    stage = "stage-at"
    parent_stage = "stage-as"
    evidence_run = 225
    evidence_compile_trace_count = 27
    family = "directshow_dvd_interface_root_headers"
...
$reportPath = Join-Path $root "minimization-stage-at.json"
$report | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $reportPath -Encoding UTF8

$metadataPath = Join-Path $root "metadata.json"
if (-not (Test-Path -LiteralPath $metadataPath -PathType Leaf)) {
    throw "LLVM-MinGW metadata missing before Stage AT: $metadataPath"
}
$metadata = Get-Content -LiteralPath $metadataPath -Raw | ConvertFrom-Json
if ([string]$metadata.minimization_stage -ne "stage-as") {
    throw "Stage AT expected Stage AS header metadata, got: $($metadata.minimization_stage)"
}
if ([string]$metadata.library_minimization_stage -ne "stage-ab") {
    throw "Stage AT expected Stage AB library metadata, got: $($metadata.library_minimization_stage)"
}
$metadata.minimization_stage = "stage-at"
$metadata.minimization_report = "minimization-stage-at.json"
...
if ($env:PHASE6_VS2022_JIT_GATE_BYTES) {
    ...
    if ($payloadBytes -gt $phase6GateBytes) {
        throw "Phase 6 size gate failed after Stage AT: staged payload $payloadBytes bytes exceeds 50% ceiling $phase6GateBytes bytes"
    }
...
Write-Host "LLVM-MinGW minimization stage-at"
Write-Host "  Stage AT removed files: $($removed.Count)"
Write-Host "  Stage AT removed MiB: $([math]::Round($actualBytes / 1MB, 2))"
Write-Host "  payload MiB after Stage AT: $([math]::Round($payloadBytes / 1MB, 2))"
