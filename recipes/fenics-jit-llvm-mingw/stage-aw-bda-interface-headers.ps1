...
# Run #230 requalified Stage AV across all 27 Phase 5 compile dependency
# closures. The Broadcast Driver Architecture interface root family below was
# unobserved in every trace: 3 files / 0.272 MiB. Use exact names and bytes so
...
$matches = @(
    foreach ($name in $names) {
        ...
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
            throw "Stage AW expected run #230 candidate header missing: $name"
        }
        Get-Item -LiteralPath $path
    }
)
if ($matches.Count -ne 3) {
    throw "Stage AW expected 3 BDA interface files from run #230 evidence, found $($matches.Count)"
}

$expectedBytes = [int64]285642
$actualBytes = [int64](($matches | Measure-Object Length -Sum).Sum)
if ($actualBytes -ne $expectedBytes) {
    throw "Stage AW expected $expectedBytes bytes from run #230 evidence, found $actualBytes; refusing changed candidate set"
}
...
foreach ($name in $requiredHeaders) {
    if (-not (Test-Path -LiteralPath (Join-Path $include $name) -PathType Leaf)) {
        throw "Stage AW safety header missing before pruning: $name"
    }
}
...
foreach ($name in $names) {
    if (Test-Path -LiteralPath (Join-Path $include $name)) {
        throw "Stage AW BDA interface file remains after pruning: $name"
    }
}
foreach ($name in $requiredHeaders) {
    if (-not (Test-Path -LiteralPath (Join-Path $include $name) -PathType Leaf)) {
        throw "Stage AW safety header missing after pruning: $name"
    }
}

$stageAVReport = Join-Path $root "minimization-stage-av.json"
if (-not (Test-Path -LiteralPath $stageAVReport -PathType Leaf)) {
    throw "Stage AV minimization report missing before Stage AW: $stageAVReport"
}

$report = [ordered]@{
    stage = "stage-aw"
    parent_stage = "stage-av"
    evidence_run = 230
    evidence_compile_trace_count = 27
    family = "broadcast_driver_architecture_interface_root_headers"
...
$reportPath = Join-Path $root "minimization-stage-aw.json"
$report | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $reportPath -Encoding UTF8

$metadataPath = Join-Path $root "metadata.json"
if (-not (Test-Path -LiteralPath $metadataPath -PathType Leaf)) {
    throw "LLVM-MinGW metadata missing before Stage AW: $metadataPath"
}
$metadata = Get-Content -LiteralPath $metadataPath -Raw | ConvertFrom-Json
if ([string]$metadata.minimization_stage -ne "stage-av") {
    throw "Stage AW expected Stage AV header metadata, got: $($metadata.minimization_stage)"
}
if ([string]$metadata.library_minimization_stage -ne "stage-ab") {
    throw "Stage AW expected Stage AB library metadata, got: $($metadata.library_minimization_stage)"
}
$metadata.minimization_stage = "stage-aw"
$metadata.minimization_report = "minimization-stage-aw.json"
...
if ($env:PHASE6_VS2022_JIT_GATE_BYTES) {
    ...
    if ($payloadBytes -gt $phase6GateBytes) {
        throw "Phase 6 size gate failed after Stage AW: staged payload $payloadBytes bytes exceeds 50% ceiling $phase6GateBytes bytes"
    }
...
Write-Host "LLVM-MinGW minimization stage-aw"
Write-Host "  Stage AW removed files: $($removed.Count)"
Write-Host "  Stage AW removed MiB: $([math]::Round($actualBytes / 1MB, 2))"
Write-Host "  payload MiB after Stage AW: $([math]::Round($payloadBytes / 1MB, 2))"
