...
# Run #221 requalified Stage AO across all 27 Phase 5 compile dependency
# closures. The DirectShow AMStream root header/IDL pair below was unobserved
# in every trace: 2 files / 0.073 MiB. Use exact names and bytes so an upstream
...
$matches = @(
    foreach ($name in $names) {
        ...
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
            throw "Stage AP expected run #221 candidate header missing: $name"
        }
        Get-Item -LiteralPath $path
    }
)
if ($matches.Count -ne 2) {
    throw "Stage AP expected 2 AMStream files from run #221 evidence, found $($matches.Count)"
}

$expectedBytes = [int64]76504
$actualBytes = [int64](($matches | Measure-Object Length -Sum).Sum)
if ($actualBytes -ne $expectedBytes) {
    throw "Stage AP expected $expectedBytes bytes from run #221 evidence, found $actualBytes; refusing changed candidate set"
}
...
foreach ($name in $requiredHeaders) {
    if (-not (Test-Path -LiteralPath (Join-Path $include $name) -PathType Leaf)) {
        throw "Stage AP safety header missing before pruning: $name"
    }
}
...
foreach ($name in $names) {
    if (Test-Path -LiteralPath (Join-Path $include $name)) {
        throw "Stage AP AMStream file remains after pruning: $name"
    }
}
foreach ($name in $requiredHeaders) {
    if (-not (Test-Path -LiteralPath (Join-Path $include $name) -PathType Leaf)) {
        throw "Stage AP safety header missing after pruning: $name"
    }
}

$stageAOReport = Join-Path $root "minimization-stage-ao.json"
if (-not (Test-Path -LiteralPath $stageAOReport -PathType Leaf)) {
    throw "Stage AO minimization report missing before Stage AP: $stageAOReport"
}

$report = [ordered]@{
    stage = "stage-ap"
    parent_stage = "stage-ao"
    evidence_run = 221
    evidence_compile_trace_count = 27
    family = "directshow_amstream_root_headers"
...
$reportPath = Join-Path $root "minimization-stage-ap.json"
$report | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $reportPath -Encoding UTF8

$metadataPath = Join-Path $root "metadata.json"
if (-not (Test-Path -LiteralPath $metadataPath -PathType Leaf)) {
    throw "LLVM-MinGW metadata missing before Stage AP: $metadataPath"
}
$metadata = Get-Content -LiteralPath $metadataPath -Raw | ConvertFrom-Json
if ([string]$metadata.minimization_stage -ne "stage-ao") {
    throw "Stage AP expected Stage AO header metadata, got: $($metadata.minimization_stage)"
}
if ([string]$metadata.library_minimization_stage -ne "stage-ab") {
    throw "Stage AP expected Stage AB library metadata, got: $($metadata.library_minimization_stage)"
}
$metadata.minimization_stage = "stage-ap"
$metadata.minimization_report = "minimization-stage-ap.json"
...
if ($env:PHASE6_VS2022_JIT_GATE_BYTES) {
    ...
    if ($payloadBytes -gt $phase6GateBytes) {
        throw "Phase 6 size gate failed after Stage AP: staged payload $payloadBytes bytes exceeds 50% ceiling $phase6GateBytes bytes"
    }
...
Write-Host "LLVM-MinGW minimization stage-ap"
Write-Host "  Stage AP removed files: $($removed.Count)"
Write-Host "  Stage AP removed MiB: $([math]::Round($actualBytes / 1MB, 2))"
Write-Host "  payload MiB after Stage AP: $([math]::Round($payloadBytes / 1MB, 2))"
