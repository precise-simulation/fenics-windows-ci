...
powershell.exe -NoProfile -Command "$meta = Get-Content -LiteralPath '%JIT_ROOT%\metadata.json' -Raw | ConvertFrom-Json; if ([string]$meta.minimization_stage -ne 'stage-al' -or [string]$meta.minimization_report -ne 'minimization-stage-al.json' -or [string]$meta.library_minimization_stage -ne 'stage-ab') { exit 1 }"
