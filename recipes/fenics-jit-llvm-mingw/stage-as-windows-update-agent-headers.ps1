...
# Run #224 requalified Stage AR across all 27 Phase 5 compile dependency
# closures. The Windows Update Agent root header/IDL pair below was unobserved
# in every trace: 2 files / 0.298 MiB. Use exact names and bytes so an upstream
...
$matches = @(
    foreach ($name in $names) {
        ...
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
            throw "Stage AS expected run #224 candidate header missing: $name"
        }
        Get-Item -LiteralPath $path
    }
)
if ($matches.Count -ne 2) {
    throw "Stage AS expected 2 Windows Update Agent files from run #224 evidence, found $($matches.Count)"
}

$expectedBytes = [int64]312833
$actualBytes = [int64](($matches | Measure-Object Length -Sum).Sum)
if ($actualBytes -ne $expectedBytes) {
    throw "Stage AS expected $expectedBytes bytes from run #224 evidence, found $actualBytes; refusing changed candidate set"
}
...
foreach ($name in $requiredHeaders) {
    if (-not (Test-Path -LiteralPath (Join-Path $include $name) -PathType Leaf)) {
        throw "Stage AS safety header missing before pruning: $name"
    }
}
...
foreach ($name in $names) {
    if (Test-Path -LiteralPath (Join-Path $include $name)) {
        throw "Stage AS Windows Update Agent file remains after pruning: $name"
    }
}
foreach ($name in $requiredHeaders) {
    if (-not (Test-Path -LiteralPath (Join-Path $include $name) -PathType Leaf)) {
        throw "Stage AS safety header missing after pruning: $name"
    }
}

$stageARReport = Join-Path $root "minimization-stage-ar.json"
if (-not (Test-Path -LiteralPath $stageARReport -PathType Leaf)) {
    throw "Stage AR minimization report missing before Stage AS: $stageARReport"
}

$report = [ordered]@{
    stage = "stage-as"
    parent_stage = "stage-ar"
    evidence_run = 224
    evidence_compile_trace_count = 27
    family = "windows_update_agent_root_headers"
...
$reportPath = Join-Path $root "minimization-stage-as.json"
$report | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $reportPath -Encoding UTF8

$metadataPath = Join-Path $root "metadata.json"
if (-not (Test-Path -LiteralPath $metadataPath -PathType Leaf)) {
    throw "LLVM-MinGW metadata missing before Stage AS: $metadataPath"
}
$metadata = Get-Content -LiteralPath $metadataPath -Raw | ConvertFrom-Json
if ([string]$metadata.minimization_stage -ne "stage-ar") {
    throw "Stage AS expected Stage AR header metadata, got: $($metadata.minimization_stage)"
}
if ([string]$metadata.library_minimization_stage -ne "stage-ab") {
    throw "Stage AS expected Stage AB library metadata, got: $($metadata.library_minimization_stage)"
}
$metadata.minimization_stage = "stage-as"
$metadata.minimization_report = "minimization-stage-as.json"
...
if ($env:PHASE6_VS2022_JIT_GATE_BYTES) {
    ...
    if ($payloadBytes -gt $phase6GateBytes) {
        throw "Phase 6 size gate failed after Stage AS: staged payload $payloadBytes bytes exceeds 50% ceiling $phase6GateBytes bytes"
    }
...
Write-Host "LLVM-MinGW minimization stage-as"
Write-Host "  Stage AS removed files: $($removed.Count)"
Write-Host "  Stage AS removed MiB: $([math]::Round($actualBytes / 1MB, 2))"
Write-Host "  payload MiB after Stage AS: $([math]::Round($payloadBytes / 1MB, 2))"
