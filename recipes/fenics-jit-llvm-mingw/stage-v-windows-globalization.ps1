...
# Run #196 requalified Stage U and measured every windows.globalization* root
# header as unobserved across all 27 Phase 5 compile dependency closures. Prune
# only this 0.272 MiB Windows Runtime subfamily; the remaining WinRT metadata
# stays intact.
$matches = @(
    Get-ChildItem -LiteralPath $include -File |
        Where-Object { $_.Name -like "windows.globalization*" } |
        Sort-Object Name
)
if ($matches.Count -eq 0) {
    throw "Stage V found no windows.globalization* headers; upstream layout may have changed"
}
...
if ($remaining.Count -ne 0) {
    throw "Stage V windows.globalization* headers remain: $($remaining.Name -join ', ')"
}

$stageUReport = Join-Path $root "minimization-stage-u.json"
if (-not (Test-Path -LiteralPath $stageUReport -PathType Leaf)) {
    throw "Stage U minimization report missing before Stage V: $stageUReport"
}

$report = [ordered]@{
    stage = "stage-v"
    parent_stage = "stage-u"
    evidence_run = 196
    evidence_compile_trace_count = 27
    family = "windows_runtime_globalization"
    patterns = @("include/windows.globalization*")
    removed_file_count = $removed.Count
    removed_bytes = $removedBytes
    removed_mib = [math]::Round($removedBytes / 1MB, 3)
    removed = $removed
}
$reportPath = Join-Path $root "minimization-stage-v.json"
$report | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $reportPath -Encoding UTF8

$metadataPath = Join-Path $root "metadata.json"
if (-not (Test-Path -LiteralPath $metadataPath -PathType Leaf)) {
    throw "LLVM-MinGW metadata missing before Stage V: $metadataPath"
}
$metadata = Get-Content -LiteralPath $metadataPath -Raw | ConvertFrom-Json
if ([string]$metadata.minimization_stage -ne "stage-u") {
    throw "Stage V expected Stage U metadata, got: $($metadata.minimization_stage)"
}
$metadata.minimization_stage = "stage-v"
$metadata.minimization_report = "minimization-stage-v.json"
...
if ($env:PHASE6_VS2022_JIT_GATE_BYTES) {
    ...
    if ($payloadBytes -gt $phase6GateBytes) {
        throw "Phase 6 size gate failed after Stage V: staged payload $payloadBytes bytes exceeds 50% ceiling $phase6GateBytes bytes"
    }
...
Write-Host "LLVM-MinGW minimization stage-v"
Write-Host "  Stage V removed files: $($removed.Count)"
Write-Host "  Stage V removed MiB: $([math]::Round($removedBytes / 1MB, 2))"
Write-Host "  payload MiB after Stage V: $([math]::Round($payloadBytes / 1MB, 2))"
