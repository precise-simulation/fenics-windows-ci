...
# Run #214 requalified Stage AH across all 27 Phase 5 compile dependency
# closures. Every retained Windows Media Player wmp* root header/IDL below was
# unobserved in every trace: 7 files / 0.432 MiB. Use exact names and bytes so
...
$matches = @(
    foreach ($name in $names) {
        ...
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
            throw "Stage AI expected run #214 candidate header missing: $name"
        }
        Get-Item -LiteralPath $path
    }
)
if ($matches.Count -ne 7) {
    throw "Stage AI expected 7 Windows Media Player files from run #214 evidence, found $($matches.Count)"
}

$expectedBytes = [int64]453484
$actualBytes = [int64](($matches | Measure-Object Length -Sum).Sum)
if ($actualBytes -ne $expectedBytes) {
    throw "Stage AI expected $expectedBytes bytes from run #214 evidence, found $actualBytes; refusing changed candidate set"
}
...
foreach ($name in $requiredHeaders) {
    if (-not (Test-Path -LiteralPath (Join-Path $include $name) -PathType Leaf)) {
        throw "Stage AI safety header missing before pruning: $name"
    }
}
...
foreach ($name in $names) {
    if (Test-Path -LiteralPath (Join-Path $include $name)) {
        throw "Stage AI Windows Media Player file remains after pruning: $name"
    }
}
foreach ($name in $requiredHeaders) {
    if (-not (Test-Path -LiteralPath (Join-Path $include $name) -PathType Leaf)) {
        throw "Stage AI safety header missing after pruning: $name"
    }
}

$stageAHReport = Join-Path $root "minimization-stage-ah.json"
if (-not (Test-Path -LiteralPath $stageAHReport -PathType Leaf)) {
    throw "Stage AH minimization report missing before Stage AI: $stageAHReport"
}

$report = [ordered]@{
    stage = "stage-ai"
    parent_stage = "stage-ah"
    evidence_run = 214
    evidence_compile_trace_count = 27
    family = "windows_media_player_root_headers"
...
$reportPath = Join-Path $root "minimization-stage-ai.json"
$report | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $reportPath -Encoding UTF8

$metadataPath = Join-Path $root "metadata.json"
if (-not (Test-Path -LiteralPath $metadataPath -PathType Leaf)) {
    throw "LLVM-MinGW metadata missing before Stage AI: $metadataPath"
}
$metadata = Get-Content -LiteralPath $metadataPath -Raw | ConvertFrom-Json
if ([string]$metadata.minimization_stage -ne "stage-ah") {
    throw "Stage AI expected Stage AH header metadata, got: $($metadata.minimization_stage)"
}
if ([string]$metadata.library_minimization_stage -ne "stage-ab") {
    throw "Stage AI expected Stage AB library metadata, got: $($metadata.library_minimization_stage)"
}
$metadata.minimization_stage = "stage-ai"
$metadata.minimization_report = "minimization-stage-ai.json"
...
if ($env:PHASE6_VS2022_JIT_GATE_BYTES) {
    ...
    if ($payloadBytes -gt $phase6GateBytes) {
        throw "Phase 6 size gate failed after Stage AI: staged payload $payloadBytes bytes exceeds 50% ceiling $phase6GateBytes bytes"
    }
...
Write-Host "LLVM-MinGW minimization stage-ai"
Write-Host "  Stage AI removed files: $($removed.Count)"
Write-Host "  Stage AI removed MiB: $([math]::Round($actualBytes / 1MB, 2))"
Write-Host "  payload MiB after Stage AI: $([math]::Round($payloadBytes / 1MB, 2))"
