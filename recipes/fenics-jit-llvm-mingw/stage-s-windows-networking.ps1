...
# Run #192 requalified Stage R. The preceding measurement identified every
# windows.networking* root header as unobserved across all 27 Phase 5 compile
# dependency closures. Prune only this 0.559 MiB Windows Runtime subfamily;
# the other WinRT metadata stays intact.
$matches = @(
    Get-ChildItem -LiteralPath $include -File |
        Where-Object { $_.Name -like "windows.networking*" } |
        Sort-Object Name
)
if ($matches.Count -eq 0) {
    throw "Stage S found no windows.networking* headers; upstream layout may have changed"
}
...
if ($remaining.Count -ne 0) {
    throw "Stage S windows.networking* headers remain: $($remaining.Name -join ', ')"
}

$stageRReport = Join-Path $root "minimization-stage-r.json"
if (-not (Test-Path -LiteralPath $stageRReport -PathType Leaf)) {
    throw "Stage R minimization report missing before Stage S: $stageRReport"
}

$report = [ordered]@{
    stage = "stage-s"
    parent_stage = "stage-r"
    evidence_run = 192
    evidence_compile_trace_count = 27
    family = "windows_runtime_networking"
    patterns = @("include/windows.networking*")
    removed_file_count = $removed.Count
    removed_bytes = $removedBytes
    removed_mib = [math]::Round($removedBytes / 1MB, 3)
    removed = $removed
}
$reportPath = Join-Path $root "minimization-stage-s.json"
$report | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $reportPath -Encoding UTF8

$metadataPath = Join-Path $root "metadata.json"
if (-not (Test-Path -LiteralPath $metadataPath -PathType Leaf)) {
    throw "LLVM-MinGW metadata missing before Stage S: $metadataPath"
}
$metadata = Get-Content -LiteralPath $metadataPath -Raw | ConvertFrom-Json
if ([string]$metadata.minimization_stage -ne "stage-r") {
    throw "Stage S expected Stage R metadata, got: $($metadata.minimization_stage)"
}
$metadata.minimization_stage = "stage-s"
$metadata.minimization_report = "minimization-stage-s.json"
...
if ($env:PHASE6_VS2022_JIT_GATE_BYTES) {
    ...
    if ($payloadBytes -gt $phase6GateBytes) {
        throw "Phase 6 size gate failed after Stage S: staged payload $payloadBytes bytes exceeds 50% ceiling $phase6GateBytes bytes"
    }
...
Write-Host "LLVM-MinGW minimization stage-s"
Write-Host "  Stage S removed files: $($removed.Count)"
Write-Host "  Stage S removed MiB: $([math]::Round($removedBytes / 1MB, 2))"
Write-Host "  payload MiB after Stage S: $([math]::Round($payloadBytes / 1MB, 2))"
