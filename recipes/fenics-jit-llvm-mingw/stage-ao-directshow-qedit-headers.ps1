...
# Run #220 requalified Stage AN across all 27 Phase 5 compile dependency
# closures. The DirectShow Editing Services qedit root header/IDL pair below
# was unobserved in every trace: 2 files / 0.106 MiB. Use exact names and
# bytes so an upstream layout change cannot silently broaden pruning.
$names = @(
    "qedit.h",
    "qedit.idl"
)
...
$matches = @(
    foreach ($name in $names) {
        ...
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
            throw "Stage AO expected run #220 candidate header missing: $name"
        }
        Get-Item -LiteralPath $path
    }
)
if ($matches.Count -ne 2) {
    throw "Stage AO expected 2 qedit files from run #220 evidence, found $($matches.Count)"
}

$expectedBytes = [int64]110912
$actualBytes = [int64](($matches | Measure-Object Length -Sum).Sum)
if ($actualBytes -ne $expectedBytes) {
    throw "Stage AO expected $expectedBytes bytes from run #220 evidence, found $actualBytes; refusing changed candidate set"
}
...
foreach ($name in $requiredHeaders) {
    if (-not (Test-Path -LiteralPath (Join-Path $include $name) -PathType Leaf)) {
        throw "Stage AO safety header missing before pruning: $name"
    }
}
...
foreach ($name in $names) {
    if (Test-Path -LiteralPath (Join-Path $include $name)) {
        throw "Stage AO qedit file remains after pruning: $name"
    }
}
foreach ($name in $requiredHeaders) {
    if (-not (Test-Path -LiteralPath (Join-Path $include $name) -PathType Leaf)) {
        throw "Stage AO safety header missing after pruning: $name"
    }
}

$stageANReport = Join-Path $root "minimization-stage-an.json"
if (-not (Test-Path -LiteralPath $stageANReport -PathType Leaf)) {
    throw "Stage AN minimization report missing before Stage AO: $stageANReport"
}

$report = [ordered]@{
    stage = "stage-ao"
    parent_stage = "stage-an"
    evidence_run = 220
    evidence_compile_trace_count = 27
    family = "directshow_editing_services_qedit_root_headers"
...
$reportPath = Join-Path $root "minimization-stage-ao.json"
$report | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $reportPath -Encoding UTF8

$metadataPath = Join-Path $root "metadata.json"
if (-not (Test-Path -LiteralPath $metadataPath -PathType Leaf)) {
    throw "LLVM-MinGW metadata missing before Stage AO: $metadataPath"
}
$metadata = Get-Content -LiteralPath $metadataPath -Raw | ConvertFrom-Json
if ([string]$metadata.minimization_stage -ne "stage-an") {
    throw "Stage AO expected Stage AN header metadata, got: $($metadata.minimization_stage)"
}
if ([string]$metadata.library_minimization_stage -ne "stage-ab") {
    throw "Stage AO expected Stage AB library metadata, got: $($metadata.library_minimization_stage)"
}
$metadata.minimization_stage = "stage-ao"
$metadata.minimization_report = "minimization-stage-ao.json"
...
if ($env:PHASE6_VS2022_JIT_GATE_BYTES) {
    ...
    if ($payloadBytes -gt $phase6GateBytes) {
        throw "Phase 6 size gate failed after Stage AO: staged payload $payloadBytes bytes exceeds 50% ceiling $phase6GateBytes bytes"
    }
...
Write-Host "LLVM-MinGW minimization stage-ao"
Write-Host "  Stage AO removed files: $($removed.Count)"
Write-Host "  Stage AO removed MiB: $([math]::Round($actualBytes / 1MB, 2))"
Write-Host "  payload MiB after Stage AO: $([math]::Round($payloadBytes / 1MB, 2))"
