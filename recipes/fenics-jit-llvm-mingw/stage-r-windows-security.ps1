...
# Run #191 requalified Stage Q. The preceding measurement identified every
# windows.security* root header as unobserved across all 27 Phase 5 compile
# dependency closures. Prune only this 0.580 MiB Windows Runtime subfamily;
# the other WinRT metadata stays intact.
$matches = @(
    Get-ChildItem -LiteralPath $include -File |
        Where-Object { $_.Name -like "windows.security*" } |
        Sort-Object Name
)
if ($matches.Count -eq 0) {
    throw "Stage R found no windows.security* headers; upstream layout may have changed"
}
...
if ($remaining.Count -ne 0) {
    throw "Stage R windows.security* headers remain: $($remaining.Name -join ', ')"
}

$stageQReport = Join-Path $root "minimization-stage-q.json"
if (-not (Test-Path -LiteralPath $stageQReport -PathType Leaf)) {
    throw "Stage Q minimization report missing before Stage R: $stageQReport"
}

$report = [ordered]@{
    stage = "stage-r"
    parent_stage = "stage-q"
    evidence_run = 191
    evidence_compile_trace_count = 27
    family = "windows_runtime_security"
    patterns = @("include/windows.security*")
    removed_file_count = $removed.Count
    removed_bytes = $removedBytes
    removed_mib = [math]::Round($removedBytes / 1MB, 3)
    removed = $removed
}
$reportPath = Join-Path $root "minimization-stage-r.json"
$report | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $reportPath -Encoding UTF8

$metadataPath = Join-Path $root "metadata.json"
if (-not (Test-Path -LiteralPath $metadataPath -PathType Leaf)) {
    throw "LLVM-MinGW metadata missing before Stage R: $metadataPath"
}
$metadata = Get-Content -LiteralPath $metadataPath -Raw | ConvertFrom-Json
if ([string]$metadata.minimization_stage -ne "stage-q") {
    throw "Stage R expected Stage Q metadata, got: $($metadata.minimization_stage)"
}
$metadata.minimization_stage = "stage-r"
$metadata.minimization_report = "minimization-stage-r.json"
...
if ($env:PHASE6_VS2022_JIT_GATE_BYTES) {
    ...
    if ($payloadBytes -gt $phase6GateBytes) {
        throw "Phase 6 size gate failed after Stage R: staged payload $payloadBytes bytes exceeds 50% ceiling $phase6GateBytes bytes"
    }
...
Write-Host "LLVM-MinGW minimization stage-r"
Write-Host "  Stage R removed files: $($removed.Count)"
Write-Host "  Stage R removed MiB: $([math]::Round($removedBytes / 1MB, 2))"
Write-Host "  payload MiB after Stage R: $([math]::Round($payloadBytes / 1MB, 2))"
