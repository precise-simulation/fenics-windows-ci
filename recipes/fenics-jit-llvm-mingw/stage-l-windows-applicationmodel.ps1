...
# Run #185 requalified Stage K and measured every windows.applicationmodel*
# header as unobserved across all 27 Phase 5 compile dependency closures.
# Prune only this 1.995 MiB Windows Runtime subfamily; the other WinRT
# metadata stays intact.
$matches = @(
    Get-ChildItem -LiteralPath $include -File |
        Where-Object { $_.Name -like "windows.applicationmodel*" } |
        Sort-Object Name
)
if ($matches.Count -eq 0) {
    throw "Stage L found no windows.applicationmodel* headers; upstream layout may have changed"
}
...
if ($remaining.Count -ne 0) {
    throw "Stage L windows.applicationmodel* headers remain: $($remaining.Name -join ', ')"
}

$stageKReport = Join-Path $root "minimization-stage-k.json"
if (-not (Test-Path -LiteralPath $stageKReport -PathType Leaf)) {
    throw "Stage K minimization report missing before Stage L: $stageKReport"
}

$report = [ordered]@{
    stage = "stage-l"
    parent_stage = "stage-k"
    evidence_run = 185
    evidence_compile_trace_count = 27
    family = "windows_runtime_applicationmodel"
    patterns = @("include/windows.applicationmodel*")
    removed_file_count = $removed.Count
    removed_bytes = $removedBytes
    removed_mib = [math]::Round($removedBytes / 1MB, 3)
    removed = $removed
}
$reportPath = Join-Path $root "minimization-stage-l.json"
$report | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $reportPath -Encoding UTF8

$metadataPath = Join-Path $root "metadata.json"
if (-not (Test-Path -LiteralPath $metadataPath -PathType Leaf)) {
    throw "LLVM-MinGW metadata missing before Stage L: $metadataPath"
}
$metadata = Get-Content -LiteralPath $metadataPath -Raw | ConvertFrom-Json
if ([string]$metadata.minimization_stage -ne "stage-k") {
    throw "Stage L expected Stage K metadata, got: $($metadata.minimization_stage)"
}
$metadata.minimization_stage = "stage-l"
$metadata.minimization_report = "minimization-stage-l.json"
$metadata | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $metadataPath -Encoding UTF8

# Recreate Stage K's manifest/size files after this additional pruning so they
...
if ($env:PHASE6_VS2022_JIT_GATE_BYTES) {
    ...
    if ($payloadBytes -gt $phase6GateBytes) {
        throw "Phase 6 size gate failed after Stage L: staged payload $payloadBytes bytes exceeds 50% ceiling $phase6GateBytes bytes"
    }
...
Write-Host "LLVM-MinGW minimization stage-l"
Write-Host "  Stage L removed files: $($removed.Count)"
Write-Host "  Stage L removed MiB: $([math]::Round($removedBytes / 1MB, 2))"
Write-Host "  payload MiB after Stage L: $([math]::Round($payloadBytes / 1MB, 2))"
