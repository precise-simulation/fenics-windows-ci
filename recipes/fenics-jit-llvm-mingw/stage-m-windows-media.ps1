...
# Run #186 requalified Stage L and measured every windows.media* header as
# unobserved across all 27 Phase 5 compile dependency closures. Prune only
# this 1.859 MiB Windows Runtime subfamily; the other WinRT metadata stays
# intact.
$matches = @(
    Get-ChildItem -LiteralPath $include -File |
        Where-Object { $_.Name -like "windows.media*" } |
        Sort-Object Name
)
if ($matches.Count -eq 0) {
    throw "Stage M found no windows.media* headers; upstream layout may have changed"
}
...
if ($remaining.Count -ne 0) {
    throw "Stage M windows.media* headers remain: $($remaining.Name -join ', ')"
}

$stageLReport = Join-Path $root "minimization-stage-l.json"
if (-not (Test-Path -LiteralPath $stageLReport -PathType Leaf)) {
    throw "Stage L minimization report missing before Stage M: $stageLReport"
}

$report = [ordered]@{
    stage = "stage-m"
    parent_stage = "stage-l"
    evidence_run = 186
    evidence_compile_trace_count = 27
    family = "windows_runtime_media"
    patterns = @("include/windows.media*")
    removed_file_count = $removed.Count
    removed_bytes = $removedBytes
    removed_mib = [math]::Round($removedBytes / 1MB, 3)
    removed = $removed
}
$reportPath = Join-Path $root "minimization-stage-m.json"
$report | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $reportPath -Encoding UTF8

$metadataPath = Join-Path $root "metadata.json"
if (-not (Test-Path -LiteralPath $metadataPath -PathType Leaf)) {
    throw "LLVM-MinGW metadata missing before Stage M: $metadataPath"
}
$metadata = Get-Content -LiteralPath $metadataPath -Raw | ConvertFrom-Json
if ([string]$metadata.minimization_stage -ne "stage-l") {
    throw "Stage M expected Stage L metadata, got: $($metadata.minimization_stage)"
}
$metadata.minimization_stage = "stage-m"
$metadata.minimization_report = "minimization-stage-m.json"
$metadata | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $metadataPath -Encoding UTF8

# Recreate Stage L's manifest/size files after this additional pruning so they
...
if ($env:PHASE6_VS2022_JIT_GATE_BYTES) {
    ...
    if ($payloadBytes -gt $phase6GateBytes) {
        throw "Phase 6 size gate failed after Stage M: staged payload $payloadBytes bytes exceeds 50% ceiling $phase6GateBytes bytes"
    }
...
Write-Host "LLVM-MinGW minimization stage-m"
Write-Host "  Stage M removed files: $($removed.Count)"
Write-Host "  Stage M removed MiB: $([math]::Round($removedBytes / 1MB, 2))"
Write-Host "  payload MiB after Stage M: $([math]::Round($payloadBytes / 1MB, 2))"
