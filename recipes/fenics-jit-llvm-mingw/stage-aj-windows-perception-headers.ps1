...
# Run #215 requalified Stage AI across all 27 Phase 5 compile dependency
# closures. Every retained Windows Runtime Perception root header/IDL below was
# unobserved in every trace: 6 files / 0.415 MiB. Use exact names and bytes so
...
$matches = @(
    foreach ($name in $names) {
        ...
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
            throw "Stage AJ expected run #215 candidate header missing: $name"
        }
        Get-Item -LiteralPath $path
    }
)
if ($matches.Count -ne 6) {
    throw "Stage AJ expected 6 Windows Runtime Perception files from run #215 evidence, found $($matches.Count)"
}

$expectedBytes = [int64]435240
$actualBytes = [int64](($matches | Measure-Object Length -Sum).Sum)
if ($actualBytes -ne $expectedBytes) {
    throw "Stage AJ expected $expectedBytes bytes from run #215 evidence, found $actualBytes; refusing changed candidate set"
}
...
foreach ($name in $requiredHeaders) {
    if (-not (Test-Path -LiteralPath (Join-Path $include $name) -PathType Leaf)) {
        throw "Stage AJ safety header missing before pruning: $name"
    }
}
...
foreach ($name in $names) {
    if (Test-Path -LiteralPath (Join-Path $include $name)) {
        throw "Stage AJ Windows Runtime Perception file remains after pruning: $name"
    }
}
foreach ($name in $requiredHeaders) {
    if (-not (Test-Path -LiteralPath (Join-Path $include $name) -PathType Leaf)) {
        throw "Stage AJ safety header missing after pruning: $name"
    }
}

$stageAIReport = Join-Path $root "minimization-stage-ai.json"
if (-not (Test-Path -LiteralPath $stageAIReport -PathType Leaf)) {
    throw "Stage AI minimization report missing before Stage AJ: $stageAIReport"
}

$report = [ordered]@{
    stage = "stage-aj"
    parent_stage = "stage-ai"
    evidence_run = 215
    evidence_compile_trace_count = 27
    family = "windows_runtime_perception_root_headers"
...
$reportPath = Join-Path $root "minimization-stage-aj.json"
$report | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $reportPath -Encoding UTF8

$metadataPath = Join-Path $root "metadata.json"
if (-not (Test-Path -LiteralPath $metadataPath -PathType Leaf)) {
    throw "LLVM-MinGW metadata missing before Stage AJ: $metadataPath"
}
$metadata = Get-Content -LiteralPath $metadataPath -Raw | ConvertFrom-Json
if ([string]$metadata.minimization_stage -ne "stage-ai") {
    throw "Stage AJ expected Stage AI header metadata, got: $($metadata.minimization_stage)"
}
if ([string]$metadata.library_minimization_stage -ne "stage-ab") {
    throw "Stage AJ expected Stage AB library metadata, got: $($metadata.library_minimization_stage)"
}
$metadata.minimization_stage = "stage-aj"
$metadata.minimization_report = "minimization-stage-aj.json"
...
if ($env:PHASE6_VS2022_JIT_GATE_BYTES) {
    ...
    if ($payloadBytes -gt $phase6GateBytes) {
        throw "Phase 6 size gate failed after Stage AJ: staged payload $payloadBytes bytes exceeds 50% ceiling $phase6GateBytes bytes"
    }
...
Write-Host "LLVM-MinGW minimization stage-aj"
Write-Host "  Stage AJ removed files: $($removed.Count)"
Write-Host "  Stage AJ removed MiB: $([math]::Round($actualBytes / 1MB, 2))"
Write-Host "  payload MiB after Stage AJ: $([math]::Round($payloadBytes / 1MB, 2))"
