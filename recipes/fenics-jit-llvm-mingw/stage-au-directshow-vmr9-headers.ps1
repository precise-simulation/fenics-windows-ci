...
# Run #226 requalified Stage AT across all 27 Phase 5 compile dependency
# closures. The DirectShow Video Mixing Renderer 9 root header/IDL pair below
# was unobserved in every trace: 2 files / 0.099 MiB. Use exact names and bytes
# so an upstream layout change cannot silently broaden pruning.
$names = @(
    "vmr9.h",
    "vmr9.idl"
)
...
$matches = @(
    foreach ($name in $names) {
        ...
        if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
            throw "Stage AU expected run #226 candidate header missing: $name"
        }
        Get-Item -LiteralPath $path
    }
)
if ($matches.Count -ne 2) {
    throw "Stage AU expected 2 DirectShow VMR9 files from run #226 evidence, found $($matches.Count)"
}

$expectedBytes = [int64]103695
$actualBytes = [int64](($matches | Measure-Object Length -Sum).Sum)
if ($actualBytes -ne $expectedBytes) {
    throw "Stage AU expected $expectedBytes bytes from run #226 evidence, found $actualBytes; refusing changed candidate set"
}
...
foreach ($name in $requiredHeaders) {
    if (-not (Test-Path -LiteralPath (Join-Path $include $name) -PathType Leaf)) {
        throw "Stage AU safety header missing before pruning: $name"
    }
}
...
foreach ($name in $names) {
    if (Test-Path -LiteralPath (Join-Path $include $name)) {
        throw "Stage AU DirectShow VMR9 file remains after pruning: $name"
    }
}
foreach ($name in $requiredHeaders) {
    if (-not (Test-Path -LiteralPath (Join-Path $include $name) -PathType Leaf)) {
        throw "Stage AU safety header missing after pruning: $name"
    }
}

$stageATReport = Join-Path $root "minimization-stage-at.json"
if (-not (Test-Path -LiteralPath $stageATReport -PathType Leaf)) {
    throw "Stage AT minimization report missing before Stage AU: $stageATReport"
}

$report = [ordered]@{
    stage = "stage-au"
    parent_stage = "stage-at"
    evidence_run = 226
    evidence_compile_trace_count = 27
    family = "directshow_vmr9_root_headers"
...
$reportPath = Join-Path $root "minimization-stage-au.json"
$report | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $reportPath -Encoding UTF8

$metadataPath = Join-Path $root "metadata.json"
if (-not (Test-Path -LiteralPath $metadataPath -PathType Leaf)) {
    throw "LLVM-MinGW metadata missing before Stage AU: $metadataPath"
}
$metadata = Get-Content -LiteralPath $metadataPath -Raw | ConvertFrom-Json
if ([string]$metadata.minimization_stage -ne "stage-at") {
    throw "Stage AU expected Stage AT header metadata, got: $($metadata.minimization_stage)"
}
if ([string]$metadata.library_minimization_stage -ne "stage-ab") {
    throw "Stage AU expected Stage AB library metadata, got: $($metadata.library_minimization_stage)"
}
$metadata.minimization_stage = "stage-au"
$metadata.minimization_report = "minimization-stage-au.json"
...
if ($env:PHASE6_VS2022_JIT_GATE_BYTES) {
    ...
    if ($payloadBytes -gt $phase6GateBytes) {
        throw "Phase 6 size gate failed after Stage AU: staged payload $payloadBytes bytes exceeds 50% ceiling $phase6GateBytes bytes"
    }
...
Write-Host "LLVM-MinGW minimization stage-au"
Write-Host "  Stage AU removed files: $($removed.Count)"
Write-Host "  Stage AU removed MiB: $([math]::Round($actualBytes / 1MB, 2))"
Write-Host "  payload MiB after Stage AU: $([math]::Round($payloadBytes / 1MB, 2))"
